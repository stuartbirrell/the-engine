...
class Help extends Command {
  ...
  async run(message, args, level) {
    ...
    if (!args[0]) {
      ...
      const myCommands = message.guild ? this.bot.commands.filter(cmd => this.bot.levelCache[cmd.conf.permLevel] <= level) : this.bot.commands.filter(cmd => this.bot.levelCache[cmd.conf.permLevel] <= level &&  cmd.conf.guildOnly !== true);
