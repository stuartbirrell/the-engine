...
class Reload extends Command {
  ...
  async run(message, args, level) { // eslint-disable-line no-unused-vars
    ...
    let response = await this.bot.unloadCommand(commands.conf.location, commands.help.name);
    if (response) return message.reply(`Error Unloading: ${response}`);

    response = this.bot.loadCommand(commands.conf.location, commands.help.name);
