...
class Set extends Command {
  ...
  async run(message, [action, key, ...value], level) { // eslint-disable-line no-unused-vars
    ...
    if (action === "edit") {
      ...
      this.bot.settings.set(message.guild.id, settings);
    ...
    if (action === "del" || action === "reset") {
      ...
      const response = await this.bot.awaitReply(message, `Are you sure you want to reset \`${key}\` to the default \`${defaults[key]}\`?`);
